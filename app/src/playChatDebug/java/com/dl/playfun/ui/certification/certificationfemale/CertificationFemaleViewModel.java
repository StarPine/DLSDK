...
import com.dl.playfun.ui.certification.goddesscertification.GoddessCertificationFragment;
...
public class CertificationFemaleViewModel extends BaseRefreshViewModel<AppRepository> {
    ...
    public BindingCommand goddessVerifyOnClickCommand = new BindingCommand(() -> {
        if (model.readUserData().getCertification() != 1) {
            ToastUtils.showShort(R.string.warn_no_certification);
            return;
        }
        if (applyGoddessStatus.get() == -1 || applyGoddessStatus.get() == 2) {
            ...
            if (applyGoddessStatus.get() == 0) {
                ToastUtils.showShort(R.string.model_certification_again);
            } else if (applyGoddessStatus.get() == 1) {
                ToastUtils.showShort(R.string.model_certifition_pass_application);
            }
        }
    });
        ...
        model.applyGoddessResult()
                ...
                .subscribe(new BaseObserver<BaseDataResponse<StatusEntity>>() {
                    ...
                    public void onSuccess(BaseDataResponse<StatusEntity> response) {
                        ...
                        switch (status) {
                            case -1:
                                applyGoddessStatusStr.set(StringUtils.getString(R.string.model_certification_not_applied));
                                break;
                            case 0:
                                applyGoddessStatusStr.set(StringUtils.getString(R.string.model_certification_wait_applied));
                                break;
                            case 1:
                                applyGoddessStatusStr.set(StringUtils.getString(R.string.model_certifition_pass));
                                break;
                            case 2:
                                applyGoddessStatusStr.set(StringUtils.getString(R.string.model_certifition_no_pass));
                                break;
                            default:
