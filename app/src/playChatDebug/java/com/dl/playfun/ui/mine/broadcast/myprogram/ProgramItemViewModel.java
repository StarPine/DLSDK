...
import static com.dl.playfun.ui.mine.broadcast.mytrends.HeadItemViewModel.Type_Topical;
    ...
    public BindingCommand moreClick = new BindingCommand(() -> {
        try {
            if (viewModel instanceof MyprogramViewModel) {
                ...
                int position = ((ProgramListViewModel) viewModel).observableList.indexOf(ProgramItemViewModel.this);
                Map<String, String> data = new HashMap<>();
                data.put("position", String.valueOf(position));
                data.put("broadcastId", String.valueOf(topicalListEntityObservableField.get().getBroadcast().getId()));
                ((ProgramListViewModel) viewModel).uc.clickMore.setValue(data);
    ...
    public BindingCommand likeClick = new BindingCommand(() -> {
        try {
            if (viewModel instanceof MyprogramViewModel) {
                ...
                int position = ((ProgramListViewModel) viewModel).observableList.indexOf(ProgramItemViewModel.this);
                ((ProgramListViewModel) viewModel).uc.clickLike.setValue(position);
            } else if (viewModel instanceof ProgramDetailViewModel) {
...
//                if (sex == MALE) {
//                    ToastUtils.showShort(R.string.warn_male_not_comment_program);
//                } else {
//                    ToastUtils.showShort(R.string.warn_female_not_comment_program);
//                }
//                return;
//            }
            if (topicalListEntityObservableField.get().getIsEnd() == 1) {
                ToastUtils.showShort(R.string.program_over);
                return;
            }
            if (viewModel instanceof MyprogramViewModel) {
                ...
                } else if (topicalListEntityObservableField.get().getUser().getSex() == sex) {
                    if (sex == MALE) {
                        ToastUtils.showShort(R.string.warn_male_not_comment_program);
                    } else {
                        ToastUtils.showShort(R.string.warn_female_not_comment_program);
                ...
                if (((ProgramListViewModel) viewModel).userId == topicalListEntityObservableField.get().getUser().getId()) {
                    ToastUtils.showShort(R.string.self_ont_comment_broadcast);
                } else if (topicalListEntityObservableField.get().getUser().getSex() == sex) {
                    if (sex == MALE) {
                        ToastUtils.showShort(R.string.warn_male_not_comment_program);
                    ...
                    data.put("toUseriD", null);
                    data.put("toUserName", null);
                    ((ProgramListViewModel) viewModel).uc.clickComment.setValue(data);
                }
            } else if (viewModel instanceof ProgramDetailViewModel) {
...
//                ToastUtils.showShort(R.string.you_have_signed_up);
//                return;
//            }
//            int sex = AppContext.instance().appRepository.readUserData().getSex();
//            if (topicalListEntityObservableField.get().getUser().getSex() == sex) {
//                if (sex == MALE) {
//                    ToastUtils.showShort(R.string.warn_male_not_sign_up_program);
//                } else {
//                    ToastUtils.showShort(R.string.warn_female_not_sign_up_program);
//                }
//                return;
//            }
            if (topicalListEntityObservableField.get().getIsEnd() == 1) {
            ...
            if (viewModel instanceof MyprogramViewModel) {
                ...
                int position = ((ProgramListViewModel) viewModel).observableList.indexOf(ProgramItemViewModel.this);
                ((ProgramListViewModel) viewModel).uc.clickSignUp.setValue(position);
            } else if (viewModel instanceof ProgramDetailViewModel) {
            ...
            try {
                if (userId == topicalListEntityObservableField.get().getUser().getId() && !isDetail.get()) {
                    ...
                    if (viewModel instanceof RadioViewModel) {
                        ...
                        int position = ((ProgramListViewModel) viewModel).observableList.indexOf(ProgramItemViewModel.this);
                        ((ProgramListViewModel) viewModel).uc.clickCheck.setValue(position);
                    } else if (viewModel instanceof ProgramDetailViewModel) {
                        int position = ((ProgramDetailViewModel) viewModel).observableList.indexOf(ProgramItemViewModel.this);
    ...
    public BindingCommand addressClick = new BindingCommand(() -> {
        try {
            if (!topicalListEntityObservableField.get().getAddressName().equals(StringUtils.getString(R.string.determined))) {
    ...
    public BindingCommand avatarClick = new BindingCommand(() -> {
        try {
            int sex = AppContext.instance().appRepository.readUserData().getSex();
            if (topicalListEntityObservableField.get().getUser().getSex() == sex) {
                ToastUtils.showShort(topicalListEntityObservableField.get().getUser().getSex() == FEMALE ? StringUtils.getString(R.string.madam_ont_check_madam_detail) : StringUtils.getString(R.string.man_ont_check_man_detail));
            } else {
...
//            sex = ((ProgramListViewModel) viewModel).sex;
            userId = ((ProgramListViewModel) viewModel).userId;
            avatar = ((ProgramListViewModel) viewModel).avatar;
        }else if (viewModel instanceof MyAllBroadcastViewModel) {
//            isVip = ((ProgramListViewModel) viewModel).isVip;
//            sex = ((ProgramListViewModel) viewModel).sex;
...
//        }
            ...
            for (int i = 0; i < topicalListEntityObservableField.get().getGive_user().size(); i++) {
                if (i < 13) {
                    HeadItemViewModel item = new HeadItemViewModel(viewModel, topicalListEntityObservableField.get().getGive_user().get(i).getAvatar(),
                            ...
                            0,
                            Type_Topical, topicalListEntityObservableField.get().getId()
                    );
                    itemList.add(item);
                } else if (i == 13) {
                    HeadItemViewModel item = new HeadItemViewModel(viewModel, topicalListEntityObservableField.get().getGive_user().get(i).getAvatar(),
    ...
    public void addGiveUser() {
        ...
        if (topicalListEntityObservableField.get().getGiveCount() < 13) {
            HeadItemViewModel item = new HeadItemViewModel(viewModel, avatar, userId,
                    ...
                    0,
                    Type_New, topicalListEntityObservableField.get().getId()
            );
            itemList.add(item);
        }
    }
    ...
    public Integer getPosition(){
        ...
        if (viewModel instanceof MyprogramViewModel) {
            ...
            position = ((ProgramListViewModel) viewModel).observableList.indexOf(ProgramItemViewModel.this);
        } else if (viewModel instanceof ProgramDetailViewModel) {
