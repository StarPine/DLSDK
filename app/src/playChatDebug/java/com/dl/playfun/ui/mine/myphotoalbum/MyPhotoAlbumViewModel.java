...
public class MyPhotoAlbumViewModel extends BaseMyPhotoAlbumViewModel<AppRepository> {
    ...
    public BindingCommand uploadPhotoOnClickCommand = new BindingCommand(new BindingAction() {
        ...
        public void call() {
            if (totalPhoto.get() == null || totalPhoto.get() >= 24) {
                ToastUtils.showShort(R.string.warn_max_upload_photo);
                return;
            }
    ...
    public MyPhotoAlbumViewModel(@NonNull Application application, AppRepository repository) {
        ...
        if (model.readUserData().getSex() == 1) {
            titleBtnText.set(StringUtils.getString(R.string.photo_album_title_button_text_male));
            stateModel.setEmptyRetryCommand(StringUtils.getString(R.string.photo_album_title_button_text_male), uploadPhotoOnClickCommand);
        } else {
            titleBtnText.set(StringUtils.getString(R.string.photo_album_title_button_female));
            stateModel.setEmptyRetryCommand(StringUtils.getString(R.string.photo_album_title_button_female), uploadPhotoOnClickCommand);
        }
