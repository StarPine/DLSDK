...
public class ImageGridView extends ViewGroup {
    ...
    public ImageGridView(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        gridSpacing = (int) a.getDimension(R.styleable.ImageGridView_ngv_gridSpacing, gridSpacing);
        singleImageSize = a.getDimensionPixelSize(R.styleable.ImageGridView_ngv_singleImageSize, singleImageSize);
        singleImageRatio = a.getFloat(R.styleable.ImageGridView_ngv_singleImageRatio, singleImageRatio);
        maxImageSize = a.getInt(R.styleable.ImageGridView_ngv_maxSize, maxImageSize);
        mode = a.getInt(R.styleable.ImageGridView_ngv_mode, mode);
        a.recycle();
...
//                gridWidth = gridHeight = (totalWidth - gridSpacing * (columnCount - 1)) / columnCount;
                ...
                gridWidth = gridHeight = (totalWidth - gridSpacing * 2) / 4;
            }
    ...
    protected void onLayout(boolean changed, int l, int t, int r, int b) {
        ...
        for (int i = 0; i < childrenCount; i++) {
            ...
            GlideEngine.createGlideEngine().loadRoundeImage(getContext(), 12, mImageInfo.get(i).thumbnailUrl, childrenView);
        }
    }
     ...
     * 设置适配器
     */
    public void setAdapter(@NonNull ImageGridViewAdapter adapter) {
        mAdapter = adapter;
        ...
        //默认是3列显示，行数根据图片的数量决定
        rowCount = imageCount / 4 + (imageCount % 4 == 0 ? 0 : 1);
        columnCount = 4;
        //grid模式下，显示4张使用2X2模式
        if (mode == MODE_GRID) {
            if (imageCount == 4) {
        ...
        if (adapter.getImageInfo().size() > maxImageSize) {
            ...
            if (child instanceof ImageGridViewWrapper) {
                ImageGridViewWrapper imageView = (ImageGridViewWrapper) child;
        ...
        if (position < imageViews.size()) {
            ...
            imageView.setOnClickListener(new OnClickListener() {
                ...
                public void onClick(View v) {
                    mAdapter.onImageItemClick(getContext(), ImageGridView.this, position, mAdapter.getImageInfo());
                }
            });
