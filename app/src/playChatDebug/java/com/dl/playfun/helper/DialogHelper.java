...
public class DialogHelper {
    ...
    public static void showCheckUserNumberDialog(BaseFragment baseFragment, int number) {
        ...
        if (AppContext.instance().appRepository.readUserData().getSex() == AppConfig.MALE) {
            if (number <= 0) {
                title = StringUtils.getString(R.string.today_browse_useup);
            } else {
                title = String.format(StringUtils.getString(R.string.today_browse_female_count), number);
            }
            toFragmentName = VipSubscribeFragment.class.getCanonicalName();
            content = String.format(StringUtils.getString(R.string.not_vip_everyday_browse_home_num), ConfigManager.getInstance().getMaxBrowseHomeNumber());
            btn = StringUtils.getString(R.string.upgrade_membership);
        } else {
            if (number <= 0) {
                title = StringUtils.getString(R.string.today_browse_useup);
            } else {
                title = String.format(StringUtils.getString(R.string.today_browse_male_count), number);
            }
            content = String.format(StringUtils.getString(R.string.not_goddess_everyday_browse_home_num), ConfigManager.getInstance().getMaxBrowseHomeNumber());
            toFragmentName = CertificationFemaleFragment.class.getCanonicalName();
            btn = StringUtils.getString(R.string.upgrade_goddess);
        }
