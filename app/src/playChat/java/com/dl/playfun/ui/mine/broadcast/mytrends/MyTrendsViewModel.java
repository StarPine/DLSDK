...
public class MyTrendsViewModel extends BaseRefreshViewModel<AppRepository> {
    ...
    public BindingCommand issuanceCommand = new BindingCommand(() -> start(PublishDynamicFragment.class.getCanonicalName()));
    ...
    public MyTrendsViewModel(@NonNull Application application, AppRepository model) {
        ...
        stateModel.setEmptyRetryCommand(StringUtils.getString(R.string.my_trends_no_issuance), null, null);
    }
    ...
    public void setComment(int posion) {
        model.setComment(observableList.get(posion).newsEntityObservableField.get().getBroadcast().getId(),
                ...
                .subscribe(new BaseObserver<BaseResponse>() {
                    ...
                    public void onSuccess(BaseResponse response) {
                        dismissHUD();
                        ToastUtils.showShort(observableList.get(posion).newsEntityObservableField.get().getBroadcast().getIsComment() == 1 ? StringUtils.getString(R.string.open_comment_success) : StringUtils.getString(R.string.close_success));
