...
//                        .setAnimationStyle(R.style.RightPopAnim)
                ...
                if (viewModel.userId == ((ProgramItemViewModel) viewModel.observableList.get(0)).topicalListEntityObservableField.get().getUser().getId()) {
                    stop.setText(((ProgramItemViewModel) viewModel.observableList.get(0)).topicalListEntityObservableField.get().getBroadcast().getIsComment() == 0 ? getString(R.string.fragment_issuance_program_no_comment) : getString(R.string.open_comment));
            ...
            public void onChanged(Object o) {
                ...
                if (viewModel.certification == 1) {
                    MVDialog.getInstance(ProgramDetailFragment.this.getContext())
                            ...
                            .setConfirmTwoText(getString(R.string.dialog_set_withdraw_account_confirm))
                            .chooseType(MVDialog.TypeEnum.CENTER)
        ...
        viewModel.uc.clickReport.observe(this, new Observer() {
            ...
            public void onChanged(Object o) {
                MVDialog.getInstance(ProgramDetailFragment.this.getContext())
                        .setContent(getString(R.string.confirm_report_title))
                        .chooseType(MVDialog.TypeEnum.CENTER)
                        .setConfirmText(getString(R.string.confirm_report))
                        .setConfirmOnlick(new MVDialog.ConfirmOnclick() {
                            @Override
                            public void confirm(MVDialog dialog) {
    ...
    private void showCoinPaySheet(boolean autoPay, String userIds) {
        new CoinPaySheet.Builder(mActivity).setPayParams(6, Integer.parseInt(userIds), getString(R.string.check_detail), autoPay, new CoinPaySheet.CoinPayDialogListener() {
            @Override
    ...
    private void vipCheckChat(Integer number, Integer coinPrice) {
        if (number <= 0) {
            ...
            MVDialog.getInstance(ProgramDetailFragment.this.getContext())
                    ...
                    .setConfirmOnlick(dialog -> {
                        dialog.dismiss();
                        new CoinPaySheet.Builder(mActivity).setPayParams(6, viewModel.userId, getString(R.string.check_detail), false, new CoinPaySheet.CoinPayDialogListener() {
                            @Override
                            public void onPaySuccess(CoinPaySheet sheet, String orderNo, Integer payPrice) {
                                sheet.dismiss();
                                ToastUtils.showShort(R.string.pay_success);
