...
import com.dl.playfun.data.source.http.observer.BaseUserDetailEmptyObserver;
...
public class UserDetailViewModel extends BaseTheirPhotoAlbumViewModel<AppRepository> {
    ...
    public BindingCommand broadcastOnClickCommand = new BindingCommand(() -> {
        ...
        Bundle bundle = ProgramDetailFragment.getStartBundle(detailEntity.get().getTopicalId());
    ...
    public BindingCommand chatOnClickCommand = new BindingCommand(() -> {
        ...
        //RxBus.getDefault().post(new UMengCustomEvent(UMengCustomEvent.EVENT_USER_DETAIL_CHAT));
...
//                        onLineDrawables();
                        ...
                        refreshDistance();
                        refreshOnlineStatus();
                        sexName.set(detailEntity.get().getSex() == 1 ? StringUtils.getString(R.string.user_sex_his) : StringUtils.getString(R.string.user_sex_her));
                        sexNameSheAndHis.set(detailEntity.get().getSex() == 1 ? StringUtils.getString(R.string.user_sex_his) : StringUtils.getString(R.string.user_sex_she));
                        showAlbum2(response.getData().getAlbum(), response.getData().getAlbumImgTotal());
                        if (model.readUserData().getSex() == 1) {
                            unLockAlbumText.set(String.format(StringUtils.getString(R.string.unlock_her_photo_album_member_free), detailEntity.get().getAlbumPayMoney()));
                        } else {
                            unLockAlbumText.set(String.format(StringUtils.getString(R.string.unlock_he_photo_album_goddess_free), detailEntity.get().getAlbumPayMoney()));
                        }
                        albumNumberText.set(String.format(StringUtils.getString(R.string.have_photos), detailEntity.get().getAlbum().size()));
    ...
    public void delLike() {
        model.deleteCollect(userId.get())
                ...
                .subscribe(new BaseObserver<BaseResponse>() {
                    ...
                    public void onSuccess(BaseResponse response) {
                        dismissHUD();
                        ToastUtils.showShort(R.string.cancel_zuizong_2);
                        if (detailEntity.get() == null) {
                            return;
                        }
                        detailEntity.get().setCollect(false);
...
//                        dismissHUD();
                ...
                .subscribe(new BaseObserver<BaseDataResponse<StatusEntity>>() {
                    ...
                    public void onSuccess(BaseDataResponse<StatusEntity> response) {
                        ...
                        if (response.getData().getStatus() == 1) {
                            ...
                            ToastUtils.showShort(R.string.no_information_cant_comment_hims);
                        } else {
                            ToastUtils.showShort(R.string.no_information_cant_comment_him);
                        }
                    }
    ...
    public void commitUserEvaluate(int tagId, String imgPath) {
        model.evaluateCreate(userId.get(), tagId, imgPath)
                ...
                .subscribe(new BaseObserver<BaseResponse>() {
                    ...
                    public void onSuccess(BaseResponse response) {
                        dismissHUD();
                        ToastUtils.showShort(R.string.user_detail_evaluation_hint);
                    }
    ...
    public void isChat(int userId, int type, int chatType) {
        model.isChat(userId)
                .doOnSubscribe(this)
                .compose(RxUtils.schedulersTransformer())
                .compose(RxUtils.exceptionTransformer())
                    ...
                    public void onSuccess(BaseDataResponse<IsChatEntity> response) {
                        if (type == 1) {//私聊他  彭石林2021-3-26修改
                            ...
                            if (response.getData().getIsChant() == 1) {
                                //d
                                getCallingInvitedInfo(chatType, userId, String.format("user_%s", userId));
                                //ChatUtils.audioCall(String.format("user_%s", userId), detailEntity.get().getNickname());
                            } else {
                                if (response.getData().getChatNumber() > 0) {
                                    uc.clickVipChat.postValue(response.getData().getChatNumber());
                                } else {
    ...
    public void addFriend() {
        ...
        if (isLinkMic) {
            getCallingInvitedInfo(1, userId.get(), String.format("user_%s", userId.get()));
//            ChatUtils.audioCall(String.format("user_%s", userId.get()), detailEntity.get().getNickname());
        } else {
            ChatUtils.chatUser(userId.get(), detailEntity.get().getNickname(), this);
        }
    }
