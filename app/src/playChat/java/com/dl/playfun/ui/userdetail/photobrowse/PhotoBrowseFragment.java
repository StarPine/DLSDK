...
public class PhotoBrowseFragment extends BaseFragment<FragmentPhotoBrowseBinding, PhotoBrowseViewModel> {
    ...
    private void payReaPackagePhoto(int photoId) {
        new CoinPaySheet.Builder(mActivity).setPayParams(4, photoId, getString(R.string.red_package_photo), false, new CoinPaySheet.CoinPayDialogListener() {
            @Override
    ...
    private void payReaPackageVideo(int photoId) {
        new CoinPaySheet.Builder(mActivity).setPayParams(5, photoId, getString(R.string.red_package_video), false, new CoinPaySheet.CoinPayDialogListener() {
            @Override
            public void onPaySuccess(CoinPaySheet sheet, String orderNo, Integer payPrice) {
                sheet.dismiss();
                ToastUtils.showShort(R.string.pay_success);
