...
public class CoinRedPackageDetailViewModel extends BaseViewModel<AppRepository> {
    ...
    public BindingCommand linkServiceOnClickCommand = new BindingCommand(new BindingAction() {
        @Override
        public void call() {
            ChatUtils.chatUser(AppConfig.CHAT_SERVICE_USER_ID, StringUtils.getString(R.string.chat_service_name), CoinRedPackageDetailViewModel.this);
        }
    });
        ...
        model.receiveCoinRedPackage(redPackageId.get())
                ...
                .subscribe(new BaseObserver<BaseResponse>() {
                    ...
                    public void onSuccess(BaseResponse response) {
                        ...
                        statusText.set(StringUtils.getString(R.string.received));
                        refreshTipText();
                    }
                ...
                .subscribe(new BaseEmptyObserver<BaseDataResponse<ChatRedPackageEntity>>(this) {
                    ...
                    public void onSuccess(BaseDataResponse<ChatRedPackageEntity> response) {
                        ...
                        if (response.getData().getStatus() == 0) {
                            if (isSender.get()) {
                                statusText.set(StringUtils.getString(R.string.to_be_received));
                            } else {
                                statusText.set(StringUtils.getString(R.string.chat_get_red_package_wait_rec));
                            }
                        } else if (response.getData().getStatus() == 1) {
                            model.saveChatCustomMessageStatus(msgId.get(), 2);
                            if (isSender.get()) {
                                statusText.set(StringUtils.getString(R.string.her_received));
                            } else {
                                statusText.set(StringUtils.getString(R.string.received));
                            }
                        } else if (response.getData().getStatus() == 2) {
                            model.saveChatCustomMessageStatus(msgId.get(), 3);
                            statusText.set(StringUtils.getString(R.string.returned));
                        }
                        refreshTipText();
    ...
    private void refreshTipText() {
        if (redpackageDetail.get().getStatus() == 1) {
            if (isSender.get()) {
                tipText.set(StringUtils.getString(R.string.you_cheated_money_recover_loss_for_you));
            } else {
                tipText.set(StringUtils.getString(R.string.redpackage_detail_tip_receiver));
            }
        } else {
            if (isSender.get()) {
                tipText.set(StringUtils.getString(R.string.redpackage_detail_tip_sender));
            } else {
                tipText.set(StringUtils.getString(R.string.redpackage_detail_tip_receiver));
            }
        }
