    ...
    public String getPhotoShowName() {
        if (viewModel.getRepository().readUserData().getSex() == 0) {
            if (itemEntity.get().getVerificationType() == 0) {
                return StringUtils.getString(R.string.audit_failed);
            } else if (itemEntity.get().getVerificationType() == -1) {
                return StringUtils.getString(R.string.photo_in_review);
            }
        }

        if (itemEntity.get().getIsRedPackage() == 1 && itemEntity.get().getIsBurn() == 1) {
            return StringUtils.getString(R.string.reading_after_burn_red_photo);
        }
        if (itemEntity.get().getIsRedPackage() == 1) {
            if (itemEntity.get().getType() == 2) {
                return StringUtils.getString(R.string.red_package_video);
            }
            return StringUtils.getString(R.string.fragment_certification_female_read_phone);
        }
        if (itemEntity.get().getIsBurn() == 1) {
            if (itemEntity.get().getType() == 2) {
                return StringUtils.getString(R.string.reading_after_burn_video);
            }
            return StringUtils.getString(R.string.reading_after_burn_photo);
        }
        return "";
    }
