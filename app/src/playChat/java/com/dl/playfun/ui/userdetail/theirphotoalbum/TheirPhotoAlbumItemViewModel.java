        ...
        if (itemEntity.get().getIsRedPackage() == 1) {
            if (itemEntity.get().getIsBurn() == 1) {
                if (itemEntity.get().getType() == 1) {
                    name = StringUtils.getString(R.string.reading_after_burn_red_photo);
                } else if (itemEntity.get().getType() == 2) {
                    name = StringUtils.getString(R.string.reading_after_burn_red_photo);
                }
                if (itemEntity.get().getBurnStatus() == 1) {
                    name = StringUtils.getString(R.string.burned);
                }
            } else {
                if (itemEntity.get().getType() == 1) {
                    name = StringUtils.getString(R.string.redpackage_photo);
                } else if (itemEntity.get().getType() == 2) {
                    name = StringUtils.getString(R.string.red_package_video);
                }
            }
        } else if (itemEntity.get().getIsBurn() == 1) {
            if (itemEntity.get().getType() == 1) {
                name = StringUtils.getString(R.string.reading_after_burn_photo);
            } else if (itemEntity.get().getType() == 2) {
                name = StringUtils.getString(R.string.reading_after_burn_video);
            }
            if (itemEntity.get().getBurnStatus() == 1) {
                name = StringUtils.getString(R.string.burned);
            }
