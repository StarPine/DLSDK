        ...
        viewModel.uc.lineEvent.observe(this, new Observer<UserDetailEntity>() {
            ...
            public void onChanged(UserDetailEntity detailEntity) {
                if (StringUtil.isEmpty(detailEntity.getWeixin()) && StringUtil.isEmpty(detailEntity.getInsgram())) {
                    binding.accountLine.setText(StringUtils.getString(R.string.no_input));
                    return;
                }
                if (detailEntity.getIsUnlockAccount() != 1) {
                    binding.accountLine.setText(StringUtils.getString(R.string.click_check_social_account));
                    return;
                }
...
//                String[] items = new String[]{getString(R.string.pull_black_shield_both_sides), getString(R.string.remark_user_title), getString(R.string.report_user_title)};
                ...
                if(viewModel.detailEntity.get().getCollect()){
                    items = new String[]{getString(R.string.pull_black_shield_both_sides), getString(R.string.report_user_title),getString(R.string.cancel_zuizong)};
                }
...
//                        start(remarkUserFragment);
                ...
                }).setCancelButton(StringUtils.getString(R.string.cancel), new BottomSheet.CancelClickListener() {
                    @Override
            ...
            dialog.setEvaluateDialogListener(new MyEvaluateDialog.EvaluateDialogListener() {
                ...
                public void onEvaluateClick(MyEvaluateDialog dialog) {
                    if (viewModel.canEvaluate.get()==false) {
                        ToastUtils.showShort(viewModel.detailEntity.get().isMale() ? R.string.no_information_cant_comment_him : R.string.no_information_cant_comment);
                        return;
                    }
                    dialog.dismiss();
                    ...
                    commitEvaluateDialog.setCommitEvaluateDialogListener((dialog12, entity) -> {
                        ...
                        if (entity != null) {
                            if (entity.isNegativeEvaluate()) {
                                MVDialog.getInstance(mActivity)
                                        .setContent(getString(R.string.provide_screenshot))
                                        .setConfirmText(getString(R.string.choose_photo))
                            ...
                            ToastUtils.showShort(R.string.user_detail_evaluation_hint2);
                        }
                    });
                }
        ...
        viewModel.uc.clickApplyCheckDetail.observe(this, new Observer<String>() {
            ...
            public void onChanged(String nickName) {
                MVDialog.getInstance(mActivity)
                        .setTitle(String.format(getString(R.string.setting_limit), nickName))
                        .setContent(getString(R.string.setting_limit_content))
                        .setConfirmText(getString(R.string.choose_photo))
                        .setConfirmOnlick(dialog -> {
                            dialog.dismiss();
                            choosePhoto();
    ...
    public void getUserdetailUnlock() {
        ...
        if (getMoreNumber.intValue() <= 2) {
            if (getMoreNumber.intValue() <= 0) {
                title1 = getString(R.string.userdetail_unlock_title1);
            } else {
                title1 = String.format(getString(R.string.userdetail_unlock_title), viewModel.detailEntity.get().getMoreNumber());
            }
            title2 = String.format(getString(R.string.pay_unlock), ConfigManager.getInstance().getGetUserHomeMoney());
        } else {
            title1 = String.format(getString(R.string.userdetail_unlock_title), viewModel.detailEntity.get().getMoreNumber());
        }
        String vipTitle = null;
        if (!ConfigManager.getInstance().isVip()) {
            vipTitle = getString(R.string.userdetail_unlock_btn_vip);
        }
        MMAlertDialog.isAlertVipMonetyunlock(UserDetailFragment.this.getContext(), true, title1, getString(R.string.userdetail_unlock_context), vipTitle, title2, new MMAlertDialog.AlertVipMonetyunlockInterface() {
...
//                    Toast.makeText(getActivity(),"中间状态",Toast.LENGTH_SHORT).show();
    ...
    private void payLockAlbum(Integer userId, String nickName, Integer coinPrice) {
        ...
        if (AppContext.instance().appRepository.readUserData().getSex() == 1) {
            btn1 = getString(R.string.to_be_member_free_albums);
        } else {
            if (AppContext.instance().appRepository.readUserData().getCertification() == 1) {
                btn1 = getString(R.string.to_be_goddess_free_albums);
            } else {
                btn1 = getString(R.string.warn_no_certification);
            }
        }
        MVDialog.getInstance(UserDetailFragment.this.getContext())
                .setContent(String.format(getString(R.string.unlock_paid_album), nickName))
                .setConfirmText(btn1)
                .setConfirmTwoText(String.format(getString(R.string.pay_ro_unlock_diamond), coinPrice))
                .setConfirmOnlick(dialog -> {
                    dialog.dismiss();
                    if (AppContext.instance().appRepository.readUserData().getSex() == 1) {
                        viewModel.start(VipSubscribeFragment.class.getCanonicalName());
                ...
                .setConfirmTwoOnclick(dialog -> {
                    dialog.dismiss();
                    new CoinPaySheet.Builder(mActivity).setPayParams(3, userId, getString(R.string.unlock_album), false, new CoinPaySheet.CoinPayDialogListener() {
                        @Override
    ...
    private void vipLockAlbum(String nickName, Integer number, Integer coinPrice) {
        if (number == 0) {
            MVDialog.getInstance(UserDetailFragment.this.getContext())
                    .setContent(String.format(getString(R.string.pay_dimond_content), coinPrice, nickName))
                    .setConfirmText(String.format(getString(R.string.pay_diamond), coinPrice))
                    .setConfirmOnlick(dialog -> {
                        dialog.dismiss();
                        new CoinPaySheet.Builder(mActivity).setPayParams(3, userId, getString(R.string.unlock_album), false, new CoinPaySheet.CoinPayDialogListener() {
                            @Override
            ...
            MVDialog.getInstance(UserDetailFragment.this.getContext())
                    .setContent(String.format(getString(R.string.use_one_chance_content), nickName, number))
    ...
    private void payCheckChat(Integer coinPrice) {
        ...
        if (sex == AppConfig.MALE) {
            title = getString(R.string.to_chat_her);
            btn1 = getString(R.string.to_be_member_free_chat);
        } else {
            title = getString(R.string.to_chat_he);
            if (AppContext.instance().appRepository.readUserData().getCertification() == 1) {
                btn1 = getString(R.string.to_be_goddess_free_chat);
            } else {
                btn1 = getString(R.string.warn_no_certification);
            }
        }
        if (!isVip()) {
            TraceDialog.getInstance(UserDetailFragment.this.getContext())
                    .setTitle(title)
                    .setConfirmText(btn1)
                    .setConfirmTwoText(String.format(getString(R.string.paid_viewing_private_chat), coinPrice))
                    .setConfirmOnlick(new TraceDialog.ConfirmOnclick() {
                        @Override
                        public void confirm(Dialog dialog) {
                            dialog.dismiss();
            ...
            MVDialog.getInstance(UserDetailFragment.this.getContext())
                    .setContent(title)
                    .setConfirmText(String.format(getString(R.string.paid_viewing_private_chat), coinPrice))
                    .setConfirmOnlick(dialog -> {
                        dialog.dismiss();
                        showCoinPaySheet(false);
                    })
    ...
    private void showCoinPaySheet(boolean autoPay) {
        new CoinPaySheet.Builder(mActivity).setPayParams(6, userId, getString(R.string.check_detail), autoPay, new CoinPaySheet.CoinPayDialogListener() {
            @Override
            public void onPaySuccess(CoinPaySheet sheet, String orderNo, Integer payPrice) {
                sheet.dismiss();
                ToastUtils.showShort(R.string.pay_success);
            ...
            public void onRechargeSuccess(CoinRechargeSheetView rechargeSheetView) {
                // 充值成功，再次唤起浮层，且自动支付
                try {
                    AppContext.runOnUIThread(rechargeSheetView::dismiss, 100);
    ...
    private void vipCheckChat(Integer number, Integer coinPrice) {
        if (number <= 0) {
            // 实际上不会走到这个分支？
            MVDialog.getInstance(UserDetailFragment.this.getContext())
                    .setContent(String.format(getString(R.string.pay_diamond_content), coinPrice))
                    .setConfirmText(String.format(getString(R.string.pay_diamond), coinPrice))
                    .setConfirmOnlick(dialog -> {
                        dialog.dismiss();
                        new CoinPaySheet.Builder(mActivity).setPayParams(6, userId, getString(R.string.check_detail), false, new CoinPaySheet.CoinPayDialogListener() {
                            @Override
                            public void onPaySuccess(CoinPaySheet sheet, String orderNo, Integer payPrice) {
                                sheet.dismiss();
                                ToastUtils.showShort(R.string.pay_success);
            ...
            //去掉使用一次機會dialog
//            MVDialog.getInstance(UserDetailFragment.this.getContext())
//                    .setContent(String.format(getString(R.string.use_one_chance_chat), number))
//                    .setConfirmText(getString(R.string.use_one_chance))
//                    .setConfirmOnlick(dialog -> {
//                        dialog.dismiss();
...
//                    .show();
    ...
    private void payUnlockChatAccount(String coinPrice) {
        ...
        if (!ConfigManager.getInstance().isVip()) {
            MVDialog.getInstance(UserDetailFragment.this.getContext())
                    .setTitle(getString(R.string.only_vip_can_unlock))
                    .setConfirmText(getString(R.string.become_vip))
                    .setConfirmOnlick(new MVDialog.ConfirmOnclick() {
                        @Override
                        public void confirm(MVDialog dialog) {
                            dialog.dismiss();
            ...
            MVDialog.getInstance(UserDetailFragment.this.getContext())
                    .setTitle(getString(R.string.unlock_account_title_dialog))
                    .setConfirmText(String.format(getString(R.string.pay_unlock), viewModel.detailEntity.get().getUnlockAccountMoney()))
                    .setConfirmOnlick(new MVDialog.ConfirmOnclick() {
                        @Override
                        public void confirm(MVDialog dialog) {
                            dialog.dismiss();
                            new CoinPaySheet.Builder(mActivity).setPayParams(11, userId, getString(R.string.unlock_social_account), false, new CoinPaySheet.CoinPayDialogListener() {
                                @Override
                                public void onPaySuccess(CoinPaySheet sheet, String orderNo, Integer payPrice) {
                                    sheet.dismiss();
    ...
    private void checkChatAccount() {
        ...
        MVDialog.getInstance(mActivity)
                .setTitleSize(16)
                .setTitle(getString(R.string.she_social_account))
                .setContent(account)
                .setConfirmText(getString(R.string.cancel))
                .setConfirmTwoText(getString(R.string.copy))
                .setConfirmTwoOnclick(dialog -> {
                    dialog.dismiss();
                    if (viewModel.detailEntity.get().getWeixin() != null && viewModel.detailEntity.get().getWeixin().length() > 0) {
                        copyStr(viewModel.detailEntity.get().getWeixin());
                    }
                    ToastUtils.showShort(R.string.copy_clipboard);
                })
                .setNotClose(true)
                .chooseType(MVDialog.TypeEnum.CENTER)
                .show();
    ...
    protected void copyStr(String text) {
        ...
        ClipData clip = ClipData.newPlainText("PlayChat", text);
