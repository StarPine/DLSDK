...
import com.dl.playfun.ui.webview.PlayGameFragment1;
...
public class MineViewModel extends BaseMyPhotoAlbumViewModel<AppRepository> {
    ...
    public BindingCommand bannerOnClickCommand = new BindingCommand(() -> {
        RxBus.getDefault().post(new UMengCustomEvent(UMengCustomEvent.EVENT_USER_CENTER_TOP_AD));
    ...
    public BindingCommand fondOnClickCommand = new BindingCommand(() -> {
        ...
        if (userInfoEntity.get().getSex() == 0) {
            ...
            bundle.putString("title", StringUtils.getString(R.string.mine_my_likes));
    ...
    public BindingCommand traceOnClickCommand = new BindingCommand(() -> {
        ...
        if (userInfoEntity.get().getSex() == 0) {
            Bundle bundle = new Bundle();
            bundle.putString("title", StringUtils.getString(R.string.mine_my_visitors_many));
    ...
    public BindingCommand certificationOnClickCommand = new BindingCommand(() -> {
        if (model.readUserData().getSex() != null) {
            ...
            if (model.readUserData().getSex() == MALE) {
                start(CertificationMaleFragment.class.getCanonicalName());
                return;
            } else if (model.readUserData().getSex() == FEMALE) {
                start(CertificationFemaleFragment.class.getCanonicalName());
                return;
            }
        }
        ToastUtils.showShort(R.string.sex_unknown);
    });
    //任务中心按钮的点击事件
    ...
    public BindingCommand memberOnClickCommand = new BindingCommand(() -> {
        ...
        RxBus.getDefault().post(new UMengCustomEvent(UMengCustomEvent.EVENT_USER_CENTER_VIP));
    ...
    public BindingCommand invitationCodeOnClickCommand = new BindingCommand(() -> {
        ...
        try {
            Bundle bundle = InviteWebDetailFragment.getStartBundle(AppConfig.API_BASE_URL + userInfoEntity.get().getInviteUrl(), userInfoEntity.get().getCode());
    ...
    public BindingCommand serviceOnClickCommand = new BindingCommand(() -> {
        try {
            ...
            ChatUtils.chatUser(AppConfig.CHAT_SERVICE_USER_ID, StringUtils.getString(R.string.chat_service_name), MineViewModel.this);
        } catch (Exception e) {
            ExceptionReportUtils.report(e);
        }
    });
    ...
    public BindingCommand complaintOnClickCommand = new BindingCommand(new BindingAction() {
        @Override
        public void call() {
            ToastUtils.showShort(R.string.mine_complaint_service);
        }
    });
    ...
    public BindingCommand toPlayerCommand2 = new BindingCommand(new BindingAction() {
        @Override
        public void call() {
            Bundle bundle = new Bundle();
            bundle.putString("webUrls", "https://avggame.shangame.com/JoyH5/");
            start(PlayGameFragment1.class.getCanonicalName(), bundle);
        }
    });
    //删除视频
    ...
    public String getAlbumPrivacy() {
        if (userInfoEntity.get().getAlbumType() == 1) {
            return StringUtils.getString(R.string.public_open);
        } else if (userInfoEntity.get().getAlbumType() == 2) {
            return String.format(StringUtils.getString(R.string.pay_ro_unlock_diamond), userInfoEntity.get().getAlbumMoney());
        } else if (userInfoEntity.get().getAlbumType() == 3) {
            return StringUtils.getString(R.string.need_verify_before_viewing);
        }
        return StringUtils.getString(R.string.unknown);
    }
    ...
    public void burnReset() {
        model.burnReset()
                ...
                .subscribe(new BaseObserver<BaseResponse>() {
                    ...
                    public void onSuccess(BaseResponse response) {
                        dismissHUD();
                        ToastUtils.showShort(R.string.restored);
