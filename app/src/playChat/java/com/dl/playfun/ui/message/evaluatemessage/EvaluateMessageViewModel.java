...
public class EvaluateMessageViewModel extends BaseRefreshViewModel<AppRepository> {
    ...
    public String getStatusText(int s) {
        String status = StringUtils.getString(R.string.unknown);
        //0未上诉 1已上诉等待处理 2上诉成功 3上诉失败
        if (s == 0) {
            status = "";
        } else if (s == 1) {
            status = StringUtils.getString(R.string.applealed);
        } else if (s == 2) {
            status = StringUtils.getString(R.string.appleal_success);
        } else if (s == 3) {
            status = StringUtils.getString(R.string.appleal_defail);
        }
        return status;
    }
