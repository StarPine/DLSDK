...
public class RoundProcess extends View {
    ...
    private float maxProgress = 100f; // 不可以修改的最大值
    //endregion
    ...
    @BindingAdapter({"round_progress"})
    public static void setRoundProgress(RoundProcess view, Integer progress) {
    ...
    @BindingAdapter({"round_max_progress"})
    public static void setRoundMaxProgress(RoundProcess view, Integer progress) {
        if (view == null || progress == null) {
            return;
        }
        view.setMaxProgress(progress);
    }
    ...
    private void initAttrs(Context context, AttributeSet attrs, int defStyleAttr) {
        TypedArray a = null;
        try {
            a = context.obtainStyledAttributes(attrs, R.styleable.RoundProcess);

            roundColor = a.getColor(R.styleable.RoundProcess_roundColor, getResources().getColor(android.R.color.darker_gray));
            roundProgressColor = a.getColor(R.styleable.RoundProcess_roundProgressColor, getResources().getColor(android.R.color.holo_red_dark));
            textColor = a.getColor(R.styleable.RoundProcess_textColor, getResources().getColor(android.R.color.holo_blue_dark));
            textSize = a.getDimension(R.styleable.RoundProcess_textSize, 22f);
            maxProgress = a.getDimension(R.styleable.RoundProcess_roundMaxProgress, 100);
