...
public class MMAlertDialog {
    ...
    public synchronized static AlertDialog showDialog(Context context,
                                                      List<VipPackageItemEntity> list,
                                                      int selected,
                                                      boolean touchOutside,
                                                      String ImageUrls,
                                                      DilodAlertInterface dilodAlertInterface) {
        AlertDialog.Builder builder = new AlertDialog.Builder(context);
        AlertDialog dialog = builder.create();
        dialog.setCanceledOnTouchOutside(touchOutside);
        dialog.setCancelable(true);
        selectedPosition = -1;//每次重新进入初始化赋值
        View view = View.inflate(context, R.layout.alert_dialog_vip, null);
        Button btn_confirm = view.findViewById(R.id.btn_confirm);
        TextView vip_detail = view.findViewById(R.id.vip_detail);
        ImageView iv_dialog_close = view.findViewById(R.id.iv_dialog_close);
        RecyclerView recyclerView = view.findViewById(R.id.vip_item);
        //背景图片
        ImageView backImage = view.findViewById(R.id.back_imgage);
        if (!StringUtils.isEmpty(ImageUrls)) {
            Glide.with(context).load(StringUtil.getFullImageUrl(ImageUrls))
                    .error(R.drawable.dialog_im_vipsub)
                    .placeholder(R.drawable.dialog_im_vipsub)
                    .dontAnimate()
        ...
        VipItemRecyclerAdapter adapter = new VipItemRecyclerAdapter(list);
        recyclerView.setAdapter(adapter);
        adapter.setDefSelect(selected);
        selectedPosition = selected;
        adapter.setOnItemListener(new VipItemRecyclerAdapter.OnItemListener() {
            @Override
            public void onClick(View v, int pos, VipPackageItemEntity vipPackageItemEntity) {
        ...
        btn_confirm.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                dilodAlertInterface.confirm(dialogFinal, DialogInterface.BUTTON_POSITIVE, selectedPosition);
            }
        });
        vip_detail.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View view) {
...
//                ToastUtils.showShort("确定");
        ...
        if (StringUtil.isEmpty(confirmTwoText)) {
            contentTowBtn.setText(context.getResources().getString(R.string.confirm));
    ...
    public static Dialog DialogAddress(Context context, boolean touchOutside, AddressEntity addressEntity, AlertAddressesInterface alertAddressesInterface) {
        ...
        if (addressEntity == null) {
            ...
            toEditAddress.setText(R.string.address_title2);
            title1.setVisibility(View.GONE);
            title2.setVisibility(View.GONE);
            title3.setVisibility(View.GONE);
        } else {
            empty.setVisibility(View.GONE);
            toEditAddress.setText(R.string.address_title3);
