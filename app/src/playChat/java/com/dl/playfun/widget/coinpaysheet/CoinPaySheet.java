package com.dl.playfun.widget.coinpaysheet;
...
public class CoinPaySheet {
    ...
    public interface CoinPayDialogListener {
        ...
        void onRechargeSuccess(CoinRechargeSheetView rechargeSheetView);
    }
...
//            }
        ...
        public void onClick(View view) {
            if (view.getId() == R.id.iv_refresh) {
                ...
                RxBus.getDefault().post(new UMengCustomEvent(UMengCustomEvent.EVENT_PAY_CONFIRM));
                showRecharge();
            }
        }

        private void pay() {
            RxBus.getDefault().post(new UMengCustomEvent(UMengCustomEvent.EVENT_PAY_RECHARGE));
            if (bottomSheet.builder.payType == PAY_TYPE_COIN_RED_PACKAGE) {
                payCoinRedPackage();
            } else {
                payOrder();
        ...
        private void payOrder() {
            if (mOrderEntity == null) {
                ToastUtils.showShort(R.string.order_no_exist);
                return;
            }
            if (mBalance < mOrderEntity.getMoney()) {
                MVDialog.getInstance(mActivity)
                        .setTitleSize(16)
                        .setTitle(StringUtils.getString(R.string.insufficient_diamonds))
                        .setContent(StringUtils.getString(R.string.top_up_to_pay))
                        .setConfirmText(StringUtils.getString(R.string.cancel))
                        .setConfirmTwoText(StringUtils.getString(R.string.dialog_set_withdraw_account_confirm))
        ...
        private void payCoinRedPackage() {
            if (mBalance < bottomSheet.builder.coinNumber) {
                MVDialog.getInstance(mActivity)
                        .setTitle(com.blankj.utilcode.util.StringUtils.getString(R.string.insufficient_diamonds))
                        .setContent(StringUtils.getString(R.string.top_up_to_pay))
                        .setConfirmText(StringUtils.getString(R.string.dialong_coin_rechaege_top_up))
                        .chooseType(MVDialog.TypeEnum.CENTER)
                        .setConfirmOnlick(dialog -> {
                            dialog.dismiss();
                            showRecharge();
