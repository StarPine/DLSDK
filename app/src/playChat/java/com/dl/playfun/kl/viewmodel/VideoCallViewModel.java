...
//            getSayHiList();
    ...
    public String ageAndConstellation(CallingInviteInfo callingInviteInfo) {
        if (callingInviteInfo != null) {
            return String.format(StringUtils.getString(R.string.age_and_constellation), callingInviteInfo.getUserProfileInfo().getAge(), callingInviteInfo.getUserProfileInfo().getConstellation());
        }
        return "";
    }
                ...
                .subscribe(new BaseObserver<BaseResponse>() {
                    ...
                    public void onSuccess(BaseResponse response) {
                        ...
                        if(isHangup){
                            ...
                            String msgText = sexText+StringUtils.getString(R.string.call_message_deatail_girl_txt4)+callingVideoInviteInfoField.get().getNickname();
                        ...
                        if (msgFromUserId.intValue() == callingVideoInviteInfoField.get().getId().intValue()) {
                            ...
                            if (isJSON2(text) && text.indexOf("type") != -1) {//做自定义通知判断
                                ...
                                if (map_data != null && map_data.get("type") != null && map_data.get("type").equals("message_gift")
                                    ...
                                    String sexText = isMale ? StringUtils.getString(R.string.call_message_deatail_girl_txt3) : StringUtils.getString(R.string.call_message_deatail_girl_txt2);
                                    String messageText = callingVideoInviteInfoField.get().getNickname() + " " + StringUtils.getString(R.string.call_message_deatail_girl_txt1) + " " + sexText + " " + giftEntity.getTitle() + "x" + giftEntity.getAmount();
                                    ...
                                    if (!isMale) {
                                        ...
                                        String itemMessage = String.format(StringUtils.getString(R.string.custom_message_txt7), giftEntity.getAmount().intValue() * giftEntity.getProfitDiamond().intValue());
                                        SpannableString itemMessageBuilder = new SpannableString(itemMessage);
                                        itemMessageBuilder.setSpan(new ForegroundColorSpan(ColorUtils.getColor(R.color.white)), 0, 4, Spannable.SPAN_EXCLUSIVE_EXCLUSIVE);
                                        itemMessageBuilder.setSpan(new ForegroundColorSpan(ColorUtils.getColor(R.color.call_message_deatail_hint1)), 4, itemMessage.length(), Spannable.SPAN_EXCLUSIVE_EXCLUSIVE);
                                        putRcvItemMessage(itemMessageBuilder, null, false);
