...
public class ProgramDetailViewModel extends BaseViewModel<AppRepository> {
        ...
        model.TopicalGive(((ProgramItemViewModel) observableList.get(0)).topicalListEntityObservableField.get().getId())
                ...
                .subscribe(new BaseObserver<BaseResponse>() {
                    ...
                    public void onSuccess(BaseResponse response) {
                        ...
                        radioadetailEvent.setRadioaType(RadioViewModel.RadioRecycleType_Topical);
    ...
    public void setComment() {
        model.setComment(((ProgramItemViewModel) observableList.get(0)).topicalListEntityObservableField.get().getBroadcast().getId(),
                ...
                .subscribe(new BaseObserver<BaseResponse>() {
                    ...
                    public void onSuccess(BaseResponse response) {
                        dismissHUD();
                        ToastUtils.showShort(((ProgramItemViewModel) observableList.get(0)).topicalListEntityObservableField.get().getBroadcast().getIsComment() == 1 ? StringUtils.getString(R.string.playfun_open_comment_success) : StringUtils.getString(R.string.playfun_close_success));
                    ...
                    public void onSuccess(BaseDataResponse<IsChatEntity> response) {
                        if (type == 1) {//私聊他  彭石林2021-3-26修改
                            ...
                            if (response.getData().getIsChant() == 1 || (model.readUserData().getSex() == AppConfig.MALE && model.readUserData().getIsVip() == 0)) {
                                ChatUtils.chatUser(String.format(StringUtils.getString(R.string.playfun_imsdk_user_id), getUserId), getNickname, ProgramDetailViewModel.this);
                            } else {
    ...
    public void addFriend() {
        ...
        ChatUtils.chatUser(String.format(StringUtils.getString(R.string.playfun_imsdk_user_id), getUserId), getNickname, this);
    }
