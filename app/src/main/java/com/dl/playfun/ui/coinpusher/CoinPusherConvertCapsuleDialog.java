...
public class CoinPusherConvertCapsuleDialog extends BaseDialog {
    ...
    private void initView() {
        ...
        binding.tvSub.setOnClickListener(v -> {
            if(SEL_COIN_PUSHER_CAPSULE!=-1){
                ...
                if(itemEntity!=null){
                    convertCoinPusherGoldsCoin(itemEntity.getValue(),itemEntity.getType());
                }
            }
        });
    }
    ...
    private void convertCoinPusherGoldsCoin(final Integer amount,Integer type){
        ConfigManager.getInstance().getAppRepository()
                .convertCoinPusherGoldsCoin(amount,type)
                .doOnSubscribe(this)
