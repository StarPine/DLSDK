    ...
    public Response intercept(Chain chain) throws IOException {
        ...
        if (!path.equals("/api/login") && !path.equals("/api/auth/login") && !path.equals("/api/register")) {
            if (localDataSource != null && localDataSource.readLoginInfo() != null && !StringUtils.isEmpty(localDataSource.readLoginInfo().getToken())) {
                ...
                //添加游戏请求头
                String gameSources = localDataSource.readKeyValue(AppConfig.GAME_SOURCES_HEADER_KEY);
                if(!StringUtils.isEmpty(gameSources)){
                    Map<String,String> mapData = GsonUtils.fromJson(gameSources, Map.class);
                    if(mapData!=null){
                        if(!StringUtils.isEmpty(mapData.get("serverId"))){
                            builder.addHeader("serverId", mapData.get("serverId").trim());
                        }
                        if(!StringUtils.isEmpty(mapData.get("roleId"))){
                            builder.addHeader("roleId", mapData.get("roleId").trim());
                        }
                    }
                }
                builder.build();
